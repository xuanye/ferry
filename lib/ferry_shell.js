...
module.exports = function shell(module, env, next) {
  ...
  if (isUndefined(remotePath))
    return error(
      `Publish environment '${env}' option 'remotePath' is not found`
    )
  if (isUndefined(tarFilename))
    return error(
      `Publish environment '${env}' option 'tarFilename' is not found`
    )
  if (isUndefined(remoteTarFileDir))
    return error(
      `Publish environment '${env}' option 'remoteTarFileDir' is not found`
    )
  if (isUndefined(ssh))
  ...
  if (
    localPathEntries &&
    !isString(localPathEntries) &&
    !isArray(localPathEntries)
  )
    return error(
      `Publish environment '${env}' option 'localPathEntries' must be a string or an array`
    )
  if (!isString(remotePath))
    return error(
      `Publish environment '${env}' option 'remotePath' must be a string`
    )
  if (!isString(tarFilename))
    return error(
      `Publish environment '${env}' option 'tarFilename' must be a string`
    )
  if (!isString(remoteTarFileDir))
    return error(
      `Publish environment '${env}' option 'remoteTarFileDir' must be a string`
    )
  if (shellTrashPath && !isString(shellTrashPath))
    return error(
      `Publish environment '${env}' option 'shellTrashPath' must be a string`
    )
  ...
  if (tag && !isString(tag) && !isFn(tag))
    return error(
      `Publish environment '${env}' option 'tag' must be a string or a function`
    )

  if (isString(localPathEntries)) localPathEntries = [localPathEntries]
  if (localPathEntries && localPathEntries.some(path.isAbsolute))
    return error(
      `Publish environment '${env}' option 'localPathEntries' must all be relative paths`
    )
  ...
  if (preCommands && !isArray(preCommands))
    return error(
      `Publish environment '${env}' option 'preCommands' must be a string or an array`
    )
  if (postCommands && !isArray(postCommands))
    return error(
      `Publish environment '${env}' option 'postCommands' must be a string or an array`
    )

  module.tag = tag
    ? isFn(tag)
      ? isString(tag(module, env, this)) ? tag(module, env, this) : md5TimeStamp
      : tag
    : md5TimeStamp
  if (!nohistory) {
  ...
  shellTrashPath = shellTrashPath || '/tmp/ferryTrashDir'
  ...
  if (nobackup && localPathEntries)
    commands.push(`mkdir -p ${remotePathBackupPath}`)
  ...
  if (!localPathEntries) {
    ...
    //在不记录历史的情况下防止历史记录丢失所以把被移走的ferry.json再拷贝回来
    if (nohistory && !merge) {
      let cpHistoryPath = path.posix.join(
        nobackup ? remotePathBackupPath : `${remotePath}.${_timeStamp}`,
        'ferry.json'
      )
      mainCommands.push(`cp ${cpHistoryPath} ferry.json`)
    }
  }
  ...
  function mainCmdFn(paths = []) {
    ...
    paths.forEach(v => {
      ...
      if (nobackup) {
        ...
        result.push(
          `${merge ? 'cp -r' : 'mv'} ${pathV} ${
            localPathEntries ? backupPath : remotePathBackupPath
          }`
        )
      } else {
  ...
  // 文件解压至远程环境父目录
  let tarCommand = `tar --no-same-owner -zmpx${
    extractVerbose ? 'v' : ''
  }f ${remoteTarFilePath}`
