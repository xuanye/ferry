...
const historyFileName = 'ferry.json'
...
module.exports = function pull(module, env, next) {
  ...
  let historyFilePath = path.join(
    module.historyFileDir || os.tmpdir(),
    `ferry${env}.json`
  )
  ...
  client.sftp((err, sftp) => {
    ...
    let readStream = sftp.createReadStream(
      path.posix.join(remotePath, historyFileName)
    )
    ...
    readStream
      .pipe(fs.createWriteStream(historyFilePath))
      .on('close', error => {
