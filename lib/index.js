    ...
    for (let i in modules) {
        ...
        if (middlewareReplace) {
            ...
            middlewareReplace.forEach(v => {
                ...
                assert(
                    ...
                    'error',
                    'New middleware must have a name, otherwise the hook function cannot mount'
                );
        ...
        let dealHook = function(hooks, direction) {
            ...
            return hooks.forEach(v => {
                let { when, fn } = v;
                assert(isString(when), 'error', "Hook function must hava a key 'when' to tell Ferry when to use this hook");
        ...
        (err, res, body) => {
            if (!err && res.statusCode === 200) {
                ...
                if (semver.lt(localVersion, latestVersion)) {
                    console.log();
                    console.log(chalk.yellow('A newer version of ferry is available.'));
                    console.log();
                    console.log('  latest:    ' + chalk.green(latestVersion));
                    console.log('  installed: ' + chalk.red(localVersion));
                    console.log();
                    console.log("Visit 'https://github.com/xuanye/ferry/releases' for more update information");
                    console.log();
                }
            }
        }
