...
const pull = require('../lib/ferry_pull.js')
const prompt = require('../lib/ferry_prompt.js')
const git = require('../lib/ferry_git.js')
const builder = require('../lib/ferry_builder.js')
const compress = require('../lib/ferry_compress.js')
const sftp = require('../lib/ferry_sftp.js')
const shell = require('../lib/ferry_shell.js')
const push = require('../lib/ferry_push.js')
const recover = require('../lib/ferry_recover.js')
...
const assert = localAssert('ferry')
const localLogger = (local, type) => (msg, end = true, cb) =>
  logger[type](msg, local, end, cb)
const warning = localLogger('ferry', 'warning')
const error = localLogger('ferry', 'error')
...
program
  ...
  .option(
    '--config <path>',
    "Set the profile path and defaults to 'ferry.config.js' in the current directory"
  )
...
program
  ...
  .action(cmd => {
    COMMAND = 'init'
    let confPath = path.join(process.cwd(), 'ferry.config.js')
      ...
      ? inquirer.prompt([
          {
            ...
            name: 'init',
            message:
              "There is a 'ferry.config.js' file in the current directory. Continue operation will overwrite. Are you sure?",
            default: false
          }
        ])
    ...
    p.then(({ init }) => {
      ...
      let fileStream = fs.createReadStream(
        path.join(__dirname, '../lib/ferry.config.js')
      )
...
program
  ...
  .action((env, cmd) => {
    ...
    assert(
      env || select || multiple,
      'error',
      "You must choose an environment to publish, please use 'ferry env <env> [option]' or 'ferry env -s|m [option]'"
    )
    let configPath = getConfigPath(cmd.parent)
    let config = require(configPath)
...
program
  ...
  .action((env, cmd) => {
    ...
    let ferry = Ferry(config, [{ env, _recover: { previous, length } }])
    ferry
      .use(pull)
      .use(recover)
      .use(push)
...
program.on('--help', function() {
  ...
  console.log('  e.g:')
  console.log('')
  console.log('    $ ferry init')
  console.log('    $ ferry list')
  console.log('    $ ferry env [env] [option]')
  console.log('    $ ferry auth <server> [option]')
  console.log('    $ ferry recover <env> [option]')
  console.log('')
})
...
if (typeof COMMAND === 'undefined') {
  error(
    "The command not selected or no exist, you can get help by enter 'ferry -h'"
  )
}
...
function MAINFUNC(config, env, cmd) {
  ...
  let ferry = Ferry(config, env)
  Object.keys(use).forEach(v => ferry.use(use[v]))
  ferry.metadata(extend)
  ferry.start(true)
}
...
function getConfigPath({ config } = {}) {
  let configPath = path.join(process.cwd(), 'ferry.config.js')
  if (config) configPath = path.resolve(config)
  if (!fs.existsSync(configPath))
    return error(
      "The configuration file does not exist. You can enter 'ferry init' to generate a reference configuration in the current directory."
    )
  return configPath
}
